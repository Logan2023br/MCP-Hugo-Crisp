  ...
  server.registerTool(
    "escalate_js_pagefly_issue",
    {
      ...
      description: `
        ...
        - CUSTOMER-SENT URL RULE — live_url MUST be a URL the CUSTOMER actually sent in chat. NEVER infer or guess it (not from the store handle, not from anywhere). The tool verifies the URL against the customer's real messages; any URL the customer did not send is rejected and the tool asks the customer for it.
