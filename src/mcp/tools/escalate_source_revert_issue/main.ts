  ...
  server.registerTool(
    "escalate_source_revert_issue",
    {
      ...
      description: `
        ...
        - CUSTOMER-SENT URL RULE — any URL you pass (each entry in screenshot_urls, plus any URL referenced inside issue_description) MUST be one the CUSTOMER actually sent in chat. NEVER infer or guess a URL (not from the store handle, not from anywhere). Any URL the customer did not send must not be included; ask the customer for it instead.
